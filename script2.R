...
hist(log10(final_data$Outpatient.Visits))
hist(log10(final_data$Inpatient.Days))

final_train <- final_data[order(final_data$county),]
...
load("C:/Users/Kuntal/Documents/GitHub/ahrf2016r/data/ahrf_county_test.rda")

dataset <- ahrf_county_test
...
states_county <- ahrf_county_test[c("F12424","F04437")]
colnames(states_county)[1] <- "state"
colnames(states_county)[2] <- "county"

states_county$county <- trimws(states_county$county)
...
final_test <- final_data[order(final_data$county),]
